...
const addBooking = async (req, res) => {
  function generateCode(length) {
    ...
    while (code.length < length) {
      var char = Math.random().toString(36).substring(2, 1);
      if (Math.random() < 0.5) {
...
const deleteBooking = async (req, res) => {
  try {
    ...
    if (!books) return res.status(404).json({ message: "Booking not found" });

    await seat.destroy({ where: { id: books.seat_id } });
...
const payBooking = async (req, res) => {
  try {
    ...
    const data = await book.findOne({ where: { booking_code: code }, include: [{ model: ticket }, { model: passenger }] });
    res.status(200).json({
      data,
    });
  } catch (error) {
